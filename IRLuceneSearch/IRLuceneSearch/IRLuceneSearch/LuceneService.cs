...
namespace IRLuceneSearch.Service
{
	...
	public class LuceneService 
    {
        ...
        private static IndexingEngine indexingEngine = new IndexingEngine();
        private static QuerySearcher querySearcher = new QuerySearcher();

        /**
         * If index in not already created then create the index
         */
        public static void CreateLuceneIndex(string AnalyzerType, out int termsCount, out int docsCount, out string status)
        {
            Analyzer analyzer = Utiliy.GetAnalyzer(AnalyzerType);
            indexingEngine.CreateLuceneIndex(analyzer, out status);
            termsCount = indexingEngine.TermsCount;
            docsCount = indexingEngine.DocsCount;
            analyzer.Close();
        }

        /**
        * If index is created, then optimize it
        */
        public static void OptimizeIndex(string AnalyzerType, out int termsCount, out int docsCount)
        {
            Analyzer analyzer = Utiliy.GetAnalyzer(AnalyzerType);
            indexingEngine.OptimizeIndex(analyzer);
            termsCount = indexingEngine.TermsCount;
            docsCount = indexingEngine.DocsCount;
            analyzer.Close();
        }
         ...
         */
        public static IEnumerable<Review> Search(string input, string latitude, string longitude, int limit)
        {
            return querySearcher.SearchQuery(input, latitude, longitude, limit);
        }
        ...
        //    var query = QueryParserClass.ParseQuery(input, latitude, longitude);
